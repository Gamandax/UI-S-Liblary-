...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
	...
	local size = subHolder and 24 or 28
	...
	local title = library:Create("TextLabel", {
		...
		BorderSizePixel = 0,
		Text = holderTitle,
		TextSize = subHolder and 12 or 14, -- Reduced font sizes from 16/17 to 12/14
	...
	local close = library:Create("ImageLabel", {
		...
		Size = UDim2.new(1, -size - 6, 1, -size - 6), -- Reduced padding from 10 to 6
	...
	layout.Changed:connect(function()
		...
		parentTable.main.Size = #parentTable.options > 0 and parentTable.open and UDim2.new(0, 180, 0, layout.AbsoluteContentSize.Y + size) or UDim2.new(0, 180, 0, size) -- Updated width
	end)
	...
	closeHolder.InputBegan:connect(function(input)
		if input.UserInputType == ui then
			...
			parentTable.main:TweenSize(#parentTable.options > 0 and parentTable.open and UDim2.new(0, 180, 0, layout.AbsoluteContentSize.Y + size) or UDim2.new(0, 180, 0, size), "Out", "Quad", 0.2, true)
		elseif input.UserInputType == Enum.UserInputType.Touch then
			parentTable.open = not parentTable.open
			tweenService:Create(close, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = parentTable.open and 90 or 180, ImageColor3 = parentTable.open and Color3.fromRGB(50, 50, 50) or Color3.fromRGB(30, 30, 30)}):Play()
			if subHolder then
...
local function createLabel(option, parent)
	local main = library:Create("TextLabel", {
		...
		Size = UDim2.new(1, 0, 0, 18), -- Reduced from 26 to 18
		BackgroundTransparency = 1,
		Text = " " .. option.text,
		TextSize = 13, -- Reduced from 17 to 13
...
function createToggle(option, parent)
	local main = library:Create("TextLabel", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 22), -- Reduced from 31 to 22
		BackgroundTransparency = 1,
		Text = " " .. option.text,
		TextSize = 13, -- Reduced from 17 to 13
		Font = Enum.Font.Gotham,
	...
	local tickboxOutline = library:Create("ImageLabel", {
		Position = UDim2.new(1, -4, 0, 3), -- Adjusted position
		Size = UDim2.new(-1, 8, 1, -6), -- Smaller checkbox
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
	...
	local tickboxInner = library:Create("ImageLabel", {
		Position = UDim2.new(0, 1, 0, 1), -- Reduced border
		Size = UDim2.new(1, -2, 1, -2), -- Reduced border
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
	...
	local checkmarkHolder = library:Create("Frame", {
		Position = UDim2.new(0, 2, 0, 2), -- Reduced padding
		Size = option.state and UDim2.new(1, -4, 1, -4) or UDim2.new(0, 0, 1, -4), -- Reduced padding
		BackgroundTransparency = 1,
		ClipsDescendants = true,
		Parent = tickboxOutline
	})
	...
	function option:SetState(state)
		...
		self.state = state
		checkmarkHolder:TweenSize(option.state and UDim2.new(1, -4, 1, -4) or UDim2.new(0, 0, 1, -4), "Out", "Quad", 0.2, true)
...
function createButton(option, parent)
	local main = library:Create("TextLabel", {
		ZIndex = 2,
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 24), -- Reduced from 34 to 24
		BackgroundTransparency = 1,
		Text = " " .. option.text,
		TextSize = 13, -- Reduced from 17 to 13
		Font = Enum.Font.Gotham,
	...
	local round = library:Create("ImageLabel", {
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.new(0.5, 0, 0.5, 0),
		Size = UDim2.new(1, -8, 1, -6), -- Reduced padding from 12/10 to 8/6
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
...
local function createBind(option, parent)
	...
	local main = library:Create("TextLabel", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 24), -- Reduced from 33 to 24
		BackgroundTransparency = 1,
		Text = " " .. option.text,
		TextSize = 13, -- Reduced from 17 to 13
		Font = Enum.Font.Gotham,
	...
	local round = library:Create("ImageLabel", {
		Position = UDim2.new(1, -4, 0, 3), -- Adjusted position
		Size = UDim2.new(0, -textService:GetTextSize(text, 12, Enum.Font.Gotham, Vector2.new(9e9, 9e9)).X - 12, 1, -8), -- Reduced font size and padding
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(40, 40, 40),
	...
	local bindinput = library:Create("TextLabel", {
		...
		BackgroundTransparency = 1,
		Text = text,
		TextSize = 12, -- Reduced from 16 to 12
		Font = Enum.Font.Gotham,
		TextColor3 = Color3.fromRGB(255, 255, 255),
		Parent = round
	})
	...
	function option:SetKey(key)
		...
		round.Size = UDim2.new(0, -textService:GetTextSize(bindinput.Text, 12, Enum.Font.Gotham, Vector2.new(9e9, 9e9)).X - 12, 1, -8)	
	end
end
...
local function createSlider(option, parent)
	local main = library:Create("Frame", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 36), -- Reduced from 50 to 36
		BackgroundTransparency = 1,
		Parent = parent.content
	})
	
	local title = library:Create("TextLabel", {
		Position = UDim2.new(0, 0, 0, 3), -- Adjusted position
		Size = UDim2.new(1, 0, 0, 14), -- Reduced from 20 to 14
		BackgroundTransparency = 1,
		Text = " " .. option.text,
		TextSize = 13, -- Reduced from 17 to 13
		Font = Enum.Font.Gotham,
	...
	local slider = library:Create("ImageLabel", {
		Position = UDim2.new(0, 6, 0, 24), -- Adjusted position, reduced from 34 to 24
		Size = UDim2.new(1, -12, 0, 4), -- Reduced padding and height from 5 to 4
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
	...
	local valueRound = library:Create("ImageLabel", {
		Position = UDim2.new(1, -4, 0, 3), -- Adjusted position
		Size = UDim2.new(0, -45, 0, 14), -- Reduced from 60/18 to 45/14
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(40, 40, 40),
		ScaleType = Enum.ScaleType.Slice,
	...
	local inputvalue = library:Create("TextBox", {
		...
		TextSize = 12, -- Reduced from 15 to 12
		TextWrapped = true,
		Font = Enum.Font.Gotham,
		Parent = valueRound
	})
	...
	main.InputBegan:connect(function(input)
		if input.UserInputType == ui then
			...
			tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(2.8, 0, 2.8, 0), ImageColor3 = Color3.fromRGB(255, 65, 65)}):Play() -- Reduced from 3.5 to 2.8
			sliding = true
			option:SetValue(option.min + ((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X) * (option.max - option.min))
		elseif input.UserInputType == Enum.UserInputType.Touch then
			tweenService:Create(fill, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255, 65, 65)}):Play()
			tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(2.8, 0, 2.8, 0), ImageColor3 = Color3.fromRGB(255, 65, 65)}):Play()
			sliding = true
			option:SetValue(option.min + ((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X) * (option.max - option.min))
		end
		if input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if not sliding then
				...
				tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(2.2, 0, 2.2, 0), ImageColor3 = Color3.fromRGB(100, 100, 100)}):Play() -- Reduced from 2.8 to 2.2
			end
		end
	end)
	...
	main.InputEnded:connect(function(input)
		if input.UserInputType == ui then
			...
			if inContact then
				tweenService:Create(fill, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(100, 100, 100)}):Play()
				tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(2.2, 0, 2.2, 0), ImageColor3 = Color3.fromRGB(100, 100, 100)}):Play()
			else
...
local function createList(option, parent, holder)
	...
	local main = library:Create("Frame", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, 36), -- Reduced from 52 to 36
		BackgroundTransparency = 1,
		Parent = parent.content
	})
	
	local round = library:Create("ImageLabel", {
		Position = UDim2.new(0, 4, 0, 3), -- Adjusted position
		Size = UDim2.new(1, -8, 1, -8), -- Reduced padding
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(40, 40, 40),
		ScaleType = Enum.ScaleType.Slice,
	...
	local title = library:Create("TextLabel", {
		Position = UDim2.new(0, 8, 0, 4), -- Adjusted position and padding
		Size = UDim2.new(1, -16, 0, 10), -- Reduced from 14 to 10
		BackgroundTransparency = 1,
		Text = option.text,
		TextSize = 11, -- Reduced from 14 to 11
		Font = Enum.Font.GothamBold,
	...
	local listvalue = library:Create("TextLabel", {
		Position = UDim2.new(0, 8, 0, 14), -- Adjusted position
		Size = UDim2.new(1, -16, 0, 18), -- Reduced from 24 to 18
		BackgroundTransparency = 1,
		Text = option.value,
		TextSize = 14, -- Reduced from 18 to 14
		Font = Enum.Font.Gotham,
		TextColor3 = Color3.fromRGB(255, 255, 255),
		TextXAlignment = Enum.TextXAlignment.Left,
		Parent = main
	...
	library:Create("ImageLabel", {
		Position = UDim2.new(1, -12, 0, 11), -- Adjusted position
		Size = UDim2.new(-1, 24, 1, -22), -- Adjusted size
		SizeConstraint = Enum.SizeConstraint.RelativeYY,
		Rotation = 90,
		BackgroundTransparency = 1,
		Image = "rbxassetid://4918373417",
	...
	option.mainHolder = library:Create("ImageButton", {
		ZIndex = 3,
		Size = UDim2.new(0, 180, 0, 36), -- Reduced from 240/52 to 180/36
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageTransparency = 1,
		ImageColor3 = Color3.fromRGB(30, 30, 30),
	...
	library:Create("UIPadding", {
		PaddingTop = UDim.new(0, 4), -- Reduced from 6 to 4
		Parent = content
	})
	...
	layout.Changed:connect(function()
		option.mainHolder.Size = UDim2.new(0, 180, 0, (valueCount > 4 and (4 * 28) or layout.AbsoluteContentSize.Y) + 8) -- Adjusted sizing
	...
	round.InputBegan:connect(function(input)
		if input.UserInputType == ui then
			...
			option.mainHolder.Position = UDim2.new(0, position.X - 3, 0, position.Y - 6) -- Adjusted position
			option.open = true
			option.mainHolder.Visible = true
			library.activePopup = option
			content.ScrollBarThickness = 6
			tweenService:Create(option.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0, Position = UDim2.new(0, position.X - 3, 0, position.Y - 2)}):Play() -- Adjusted
			tweenService:Create(option.mainHolder, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0.1), {Position = UDim2.new(0, position.X - 3, 0, position.Y + 1)}):Play()
		...
		elseif input.UserInputType == Enum.UserInputType.Touch then
			...
			local position = main.AbsolutePosition
			option.mainHolder.Position = UDim2.new(0, position.X - 3, 0, position.Y - 6)
			option.open = true
			option.mainHolder.Visible = true
			library.activePopup = option
			content.ScrollBarThickness = 6
			tweenService:Create(option.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0, Position = UDim2.new(0, position.X - 3, 0, position.Y - 2)}):Play()
			tweenService:Create(option.mainHolder, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0.1), {Position = UDim2.new(0, position.X - 3, 0, position.Y + 1)}):Play()
			for _,label in next, content:GetChildren() do
				if label:IsA"TextLabel" then
					tweenService:Create(label, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0, TextTransparency = 0}):Play()
	...
	function option:Close()
		...
		local position = main.AbsolutePosition
		tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = inContact and Color3.fromRGB(60, 60, 60) or Color3.fromRGB(40, 40, 40)}):Play()
		tweenService:Create(self.mainHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1, Position = UDim2.new(0, position.X - 3, 0, position.Y - 6)}):Play()
		for _,label in next, content:GetChildren() do
			if label:IsA"TextLabel" then
...
local function createBox(option, parent)
	...
	local round = library:Create("ImageLabel", {
		Position = UDim2.new(0, 6, 0, 5), -- Adjusted position
		Size = UDim2.new(1, -12, 1, -12), -- Adjusted size
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageColor3 = Color3.fromRGB(20, 20, 20),
		ScaleType = Enum.ScaleType.Slice,
	...
	local title = library:Create("TextLabel", {
		Position = UDim2.new(0, 8, 0, 4), -- Adjusted position
		Size = UDim2.new(1, -16, 0, 10), -- Reduced from 14 to 10
		BackgroundTransparency = 1,
		Text = option.text,
		TextSize = 11, -- Reduced from 14 to 11
...
local function createColorPickerWindow(option)
	option.mainHolder = library:Create("ImageButton", {
		ZIndex = 3,
		Size = UDim2.new(0, 180, 0, 130), -- Reduced from 240/180 to 180/130
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageTransparency = 1,
		ImageColor3 = Color3.fromRGB(30, 30, 30),
	...
	option.hue = library:Create("ImageLabel", {
		...
		Position = UDim2.new(0, 6, 1, -6), -- Adjusted position
		Size = UDim2.new(1, -75, 0, 16), -- Reduced from -100/22 to -75/16
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageTransparency = 1,
		ScaleType = Enum.ScaleType.Slice,
	...
	option.satval = library:Create("ImageLabel", {
		ZIndex = 3,
		Position = UDim2.new(0, 6, 0, 6), -- Adjusted position
		Size = UDim2.new(1, -75, 1, -30), -- Reduced size
		BackgroundTransparency = 1,
	...
	option.satvalSlider = library:Create("Frame", {
		...
		Size = UDim2.new(0, 3, 0, 3), -- Reduced from 4 to 3
		Rotation = 45,
		BackgroundTransparency = 1,
	...
	option.visualize2 = library:Create("ImageLabel", {
		ZIndex = 3,
		Position = UDim2.new(1, -6, 0, 6), -- Adjusted position
		Size = UDim2.new(0, -60, 0, 60), -- Reduced from -80/80 to -60/60
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
	...
	option.resetColor = library:Create("ImageLabel", {
		ZIndex = 3,
		Position = UDim2.new(1, -6, 0, 70), -- Adjusted position
		Size = UDim2.new(0, -60, 0, 12), -- Reduced from -80/18 to -60/12
		BackgroundTransparency = 1,
		Image = "rbxassetid://3570695787",
		ImageTransparency = 1,
		ImageColor3 = Color3.fromRGB(20, 20, 20),
	...
	option.resetText = library:Create("TextLabel", {
		...
		TextTransparency = 1,
		Font = Enum.Font.Code,
		TextSize = 11, -- Reduced from 15 to 11
		TextColor3 = Color3.fromRGB(255, 255, 255),
